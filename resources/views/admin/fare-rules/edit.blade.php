...
<div class="d-flex justify-content-between align-items-center mb-4">
    <h2 class="page-title"><i class="fas fa-coins me-2"></i>แก้ไขกฎค่าโดยสาร</h2>
...
<div class="card">
    <div class="card-body">
        <form action="{{ route('admin.fare-rules.update', $fareRule) }}" method="POST">
            @csrf
            @method('PUT')
            ...
            <div class="row">
                <div class="col-md-6 mb-3">
                    ...
                    <input type="text" class="form-control @error('name') is-invalid @enderror" 
                           id="name" name="name" value="{{ old('name', $fareRule->name) }}" required>
                    @error('name')
                ...
                <div class="col-md-6 mb-3">
                    ...
                    <select class="form-select @error('type') is-invalid @enderror" 
                        ...
                        <option value="fixed" {{ old('type', $fareRule->type) == 'fixed' ? 'selected' : '' }}>Fixed (ค่าคงที่)</option>
                        <option value="distance_based" {{ old('type', $fareRule->type) == 'distance_based' ? 'selected' : '' }}>Distance Based (คำนวณตามระยะทาง)</option>
                        <option value="special" {{ old('type', $fareRule->type) == 'special' ? 'selected' : '' }}>Special (พิเศษ)</option>
                    </select>
                    @error('type')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
            ...
            <div class="row">
                <div class="col-md-6 mb-3">
                    ...
                    <select class="form-select @error('route_id') is-invalid @enderror" 
                        ...
                        @foreach($routes as $route)
                            <option value="{{ $route->id }}" {{ old('route_id', $fareRule->route_id) == $route->id ? 'selected' : '' }}>
                                {{ $route->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="col-md-6 mb-3">
                    ...
                    <input type="number" step="0.01" class="form-control @error('base_fare') is-invalid @enderror" 
                           id="base_fare" name="base_fare" value="{{ old('base_fare', $fareRule->base_fare) }}">
                    @error('base_fare')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
            ...
            <div class="row">
                <div class="col-md-6 mb-3">
                    ...
                    <select class="form-select @error('calculation_mode') is-invalid @enderror" 
                        ...
                        <option value="per_passenger" {{ old('calculation_mode', $fareRule->calculation_mode) == 'per_passenger' ? 'selected' : '' }}>ต่อผู้โดยสาร</option>
                        <option value="per_trip" {{ old('calculation_mode', $fareRule->calculation_mode) == 'per_trip' ? 'selected' : '' }}>ต่อรอบ</option>
                        <option value="per_km" {{ old('calculation_mode', $fareRule->calculation_mode) == 'per_km' ? 'selected' : '' }}>ต่อกิโลเมตร</option>
                    </select>
                ...
                <div class="col-md-3 mb-3">
                    ...
                    <input type="date" class="form-control @error('effective_from') is-invalid @enderror" 
                           id="effective_from" name="effective_from" value="{{ old('effective_from', $fareRule->effective_from->format('Y-m-d')) }}" required>
                ...
                <div class="col-md-3 mb-3">
                    ...
                    <input type="date" class="form-control @error('effective_until') is-invalid @enderror" 
                           id="effective_until" name="effective_until" value="{{ old('effective_until', $fareRule->effective_until?->format('Y-m-d')) }}">
            ...
            <div class="mb-3">
                ...
                <textarea class="form-control @error('description') is-invalid @enderror" 
                          id="description" name="description" rows="3">{{ old('description', $fareRule->description) }}</textarea>
                @error('description')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div class="mb-3">
                <div class="form-check form-switch">
                    <input class="form-check-input" type="checkbox" id="is_active" name="is_active" 
                           value="1" {{ old('is_active', $fareRule->is_active) ? 'checked' : '' }}>
